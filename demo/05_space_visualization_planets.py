...
#points, dims = generators.generate_cone(phi, z=np.array([0, 1.0]), theta=np.pi/6, hole_radius=0.5, r_min=0.25)
points, dims = generators.generate_sphere(phi, theta, r=np.array([0.5, 1.0]))
#points, dims = generators.generate_cylinder(phi, z=np.array([0, 1.0]), r_outer=0.5, hole=0.0)
lunohod.set_points(points, dims)
